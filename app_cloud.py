...
# IMPORTANT: Do not hard-code API keys in source code.  This cloud app
# expects the OPENAI_API_KEY environment variable to be set in the
# deployment environment (Railway, etc.) to authenticate calls to OpenAI.
from chromadb.config import Settings
from typing import List, Dict, Any

# Import CrossEncoder for two-stage retrieval with re-ranking.  See
# requirements.txt for the sentence-transformers dependency.  Cross-encoder
# models jointly encode the query and passage to produce more accurate
# relevance scores than embedding similarity alone【442094950299547†L115-L153】.
...
with st.sidebar:
    ...
    # Increase the retrieval range to allow the cross-encoder re-ranker
    # more candidates.  Default of 6 strikes a balance between recall and
    # performance.
    top_k = st.slider("Results to retrieve", 1, 12, 6)
...
def search(query: str, k: int = 4) -> List[Dict[str, Any]]:
    """
    Perform a two-stage retrieval: first fetch a larger candidate set, then
    optionally re-rank using a cross-encoder for better relevance.  This
    returns up to k documents with their text and metadata.
    """
    # Retrieve more candidates than requested to allow the re-ranker to
    # choose the best.  Limit to avoid excessive load.
    n_candidates = min(k * 3, 50)
    raw_docs = vectordb.similarity_search(query, n_candidates)
    if not raw_docs:
        return []
    # Load cross-encoder once at function scope to avoid repeated model loads.
    # If loading fails, we skip re-ranking.
